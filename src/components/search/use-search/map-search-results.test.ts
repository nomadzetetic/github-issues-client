...
export const getSearchResultsMock = (hasNextPage: boolean, endCursor: string) =>
  ({
    search: {
      nodes: [
        {
...
test('map search result with cursor', () => {
  const results = getSearchResultsMock(true, 'Y3Vyc29yOjEw');
  expect(mapSearchResults(results)).toEqual({
    cursor: 'Y3Vyc29yOjEw',
    issues: results.search.nodes,
  });
});

test('map search results without cursor', () => {
  const results = getSearchResultsMock(false, 'Y3Vyc29yOjEw');
  expect(mapSearchResults(results)).toEqual({
    cursor: '',
    issues: results.search.nodes,
  });
